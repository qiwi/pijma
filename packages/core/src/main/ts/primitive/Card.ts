import { Interpolation, styled, StyledOptions, Theme } from '../styled'
import { Box, BoxProps } from './Box'
import { cssValue, customScroll, Value } from './Value'
...
export const CardNonProps: PropertyKey[] = [
  'bg',
  'b',
  'bt',
  'br',
...
export const CardStyles: Interpolation<
  BoxProps & CardProps & { theme: Theme }
> = ({ theme, ...props }) => ({
  background: props.bg,
  border: props.b,
  borderTop: props.bt,
  ...
  ...(customScroll
    ? {
        ...
        '&::-webkit-scrollbar-button:single-button': {
          ...
          display: 'block',
          height: `calc(${
            cssValue(props.py || props.pt || props.p, theme.scale) || '8px'
          } - 8px)`,
          width: `calc(${
            cssValue(props.px || props.pl || props.p, theme.scale) || '8px'
          } - 8px)`,
        },
